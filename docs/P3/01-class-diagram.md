...
因此，在后续实现中适合为不同任务类别设计独立的校验策略，例如：
...
这些通知虽然都属于 `Notification`，但标题、内容模板、接收人和触发来源并不相同。因此，在后续实现中适合通过工厂模式统一通知对象创建过程，例如：
...
这两种模式都不是为了“显得高级”而强行套用，而是因为它们正好对应了 CampusHub 中最真实、最容易在实现阶段变复杂的两个位置。
