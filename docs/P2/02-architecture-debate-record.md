...
> 我们是 3 人学生团队，10 周内完成 CampusHub 校园互助服务平台 MVP。功能包括学校邮箱注册登录、需求发布、任务大厅、接单与订单状态流转、订单内留言、评价信用、举报审核和基础后台。技术栈已确定为前端 Vue3、后端 Java Spring Boot、数据库 MySQL。不做在线支付、推荐算法、完整即时通讯，不希望引入 Redis、消息队列、网关等中间件。请推荐合适架构，并说明为什么不选微服务。

**AI 回答摘要：**

AI 推荐前后端分离的分层单体架构：Vue3 负责页面与状态管理，Spring Boot 提供 RESTful API，后端按 Controller、Service、Repository 分层，并按用户、需求、订单、消息、评价、举报、后台等模块组织代码。MySQL 负责持久化，通知使用站内消息表和轮询，过期需求使用 Spring 定时任务。AI 明确指出微服务对 3 人团队和 10 周 MVP 过重。
...
> 请你扮演一名偏务实的学生项目架构师，为 CampusHub 做架构评审。请重点检查：1）订单状态一致性；2）用户权限和隐私；3）无需中间件时通知和搜索如何实现；4）后续如果扩展怎么演进。请不要推荐超出学生团队能力的复杂方案。

**AI 回答摘要：**

AI 建议采用模块化单体，并把订单作为核心领域模块处理。订单状态变更必须通过统一的 OrderService 完成，使用数据库事务和状态枚举避免非法流转。权限方面使用 Spring Security + JWT，敏感信息前端脱敏。通知使用 notification 表和前端轮询实现，搜索先用 MySQL 索引和 LIKE/全文索引。后续可从文件、通知、搜索等非核心模块逐步演进。
...
| 是否需要消息队列 | 接单、评价、举报等流程需要可追踪的一致性。通知量小，可用数据库通知表和轮询。 | 不引入消息队列。 |
