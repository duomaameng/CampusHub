...
| 消息通知模块 | 接单申请通知、订单状态变更通知、评价通知、举报处理结果通知、订单内文字留言、未读消息数量、消息已读/删除、消息内容关键词过滤；P1 用户故事中的图片消息作为文件模块协作的扩展项 | notification、order_message |
...
| 消息与通知 | FR-MSG-001 ~ FR-MSG-005、US-07、US-08、US-12 | 消息通知模块；用户与认证模块负责黑名单关系；文件上传模块支撑图片消息扩展 |
